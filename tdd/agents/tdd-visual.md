...
**A-0. Fixture URL 감지 (Route Pages):**

Route Page Container 컴포넌트인 경우, fixture URL 사용을 먼저 시도한다.

1. 대상 컴포넌트가 Container(Route Page)인지 확인: prompt의 `components`에서 `[Container]` 태그 확인
2. `_fixtures/` 디렉토리 존재 확인: `Glob("**/_fixtures/*.ts")`
3. Design context에 Fixture Map이 있는지 확인

세 조건 모두 충족 시:
- `url-fixture-pattern` skill을 Glob으로 검색하여 참조: `Glob("**/url-fixture-pattern/SKILL.md")`
- 각 fixture scenario에 대해 `{dev_url}/{route}?fixture={scenario_name}` URL 목록 생성
- 이 URL들을 preview_url로 사용 → Storybook/Preview 페이지 생성 건너뜀
- Visual Contract의 각 State가 fixture scenario에 매핑됨

하나라도 미충족 → 아래 A-1/A-2 기존 로직으로 진행.
...
**A-2. Storybook 미존재 시 → 격리된 Preview 페이지 생성:**
